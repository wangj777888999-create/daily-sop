...
| 路径 | 推荐顺序 | 适合谁 |
|---|---|---|
| **快速通读**（半天）| 01 → 05 → 06 → 11 | 想 1 小时建立全景、快速进入 vibe coding |
| **前端方向**（1–2 天）| 01 → 02 → 03 → 05 → 10 → 11 | 偏前端 / UI / 设计系统 |
| **后端方向**（1–2 天）| 01 → 04 → 05 → 06 → 07 → 08 → 09 | 偏数据、AST、沙箱、RAG |
| **AI 应用方向**（2–3 天）| 01 → 09 → 10 → 11 → 12 | 想做 AI 产品/RAG/Agent |
| **彻底通读**（3–5 天）| 00 → 12 顺序 | 想把这套技能整体内化 |
...
| 06 | [SOP 引擎与 AST](./06-sop-engine-ast.md) | Python 代码 ↔ SOP 结构化的 round-trip |
| 07 | [沙箱与执行](./07-sandbox-and-execution.md) | 子进程、超时、文件路径注入；安全边界到哪 |
